...
int main(int argc, char **argv) {
  char *server = ":Angel!wings@irc.org PRIVMSG Wiz :Are you receiving this message ?\r\nPRIVMSG Angel :yes I'm receiving it !\r\n";
  irc_parser parser;

  irc_parser_init(&parser);

  irc_parser_on_nick(&parser, nick);
  irc_parser_on_name(&parser, name);
  irc_parser_on_host(&parser, host);
  irc_parser_on_command(&parser, command);
  irc_parser_on_param(&parser, param);
  irc_parser_on_end(&parser, end);
